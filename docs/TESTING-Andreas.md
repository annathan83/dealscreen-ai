...
## 7. Why does the site load slowly? (local dev)

If the first load or every load feels slow, common causes:

| Cause | What to do |
|-------|------------|
| **OneDrive (or other sync)** | The project is under `OneDrive\Apps\...`. Sync can make file I/O very slow. **Try cloning or moving the repo to a non-synced folder** (e.g. `C:\dev\dealhub-ai`) and run `npm run dev` from there. Often this is the biggest win on Windows. |
| **First request in dev** | Next.js dev compiles routes on demand. The first time you hit `/` or `/deals` it compiles that route and dependencies; the next request is faster until you change code. Normal for dev. |
| **Production build** | For a faster experience, run `npm run build` then `npm run start`. Production is precompiled and much snappier. |
| **Supabase round-trips** | Each protected page does auth + data (middleware checks user, then the page fetches deals). Slow Supabase or high latency will add up. Check your Supabase region vs your location. |

The app already uses `loading.tsx` on deal routes so the shell and skeleton show while data loads. If the delay is before any content appears, OneDrive or first-compile is the usual culprit.
